...
>
> **MVP 范围**：MVP 阶段仅涉及 C++ 渲染引擎部分。Java 控制面板相关的工具链、依赖、构建、测试均为后期内容。
...
| OpenAL Soft | 1.25.1 | 音频播放与混合 | 系统包（`libopenal-dev`） | 后期 |
| IXWebSocket | 11.4.6 | WebSocket 服务端（含 TLS 支持） | 源码编译（CMake subdirectory） | 后期 |
...
> **后期增加依赖**：`libopenal-dev`（音频）、`libssl-dev`（WebSocket TLS）。
...
后期引入音频、WebSocket、Java 控制面板后的完整目录：
...
| 场景 | 验证内容 |
|:---|:---|
| WebSocket 通信 | 控制面板 → WebSocket 握手 → 指令收发 |
