...
//      the 7 startup commands IN ORDER (load_model, set_position, set_size,
//      set_opacity, set_fps, set_volume, set_layout) via the Protocol (T9)
//      factories. Each command is serialized compact and pushed through the
//      configured CommandSender (wired to WsServer::sendText in production).
...
//
// NOT sent here:
//   - `set_scale` — a renderer-side compatibility alias for set_layout's scale
//     axis (now implemented, but redundant for this controller). Scale is
//     carried by `set_layout`'s `scale` field, so the salvo sends set_layout.
//     The "no set_scale" rule is a PRIMARY acceptance criterion and is
//     asserted in StartupSalvoTest.
//   - Any runtime command (play_motion, etc.) — those land via the runtime
//     dispatcher, not the bootstrap salvo.
//
...
// (T18). The salvo reads only the 12 fields it needs to build the 7 commands.
...
// 7 commands need so the salvo's input surface is auditable in isolation.
...
public:
    ...
    // Send the 7 startup commands (called once after the `ready` event).
